...
.NOTES
    ...
    Version history:
    26.08.07: Forced the import of the Hyper-V PowerShell module 
    26.06.10: Removed VM Configuration Version
...
$VMConfigs = @(
    @{
        VMPath              = "$($tsenv:DriveLetter)\$($tsenv:VMName)"
        VMName              = $tsenv:VMName
        SecureBoot          = "True"
        TPM                 = "False"
        Memory              = $tsenv:RamToAssign
        Processors          = "8"
        DiskSize            = "80"
        SecondDiskSize      = "500"
        DVD                 = "True"
        VMSwitch            = $tsenv:VMSwitch
...
foreach ($config in $VMConfigs) {
    ...
    try {
        ...
        $hostCores = (Get-CimInstance Win32_Processor | ForEach-Object { $_.NumberOfLogicalProcessors ?? $_.NumberOfCores } | Measure-Object -Sum).Sum
        If ($hostCores -lt $processors) {$processors = $hostCores}
        ...
        Invoke-Expression "`${tsenv:2PINT-LABKIT-primaryVhd} = `$primaryVhd"
