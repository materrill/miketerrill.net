<#
.SYNOPSIS
    Downloads and installs Microsoft SQL Server 2025 Express (x64) on Windows.
.DESCRIPTION
    This script automates the download, silent installation, and verification of SQL Server 2025 Express.
...
# Configuration  
$DownloadUrl = "https://go.microsoft.com/fwlink/p/?linkid=2216019"  # Official Microsoft URL for SQL Server 2025 Express
$InstallerPath = "C:\Windows\Temp\SQL2025-SSEI-Expr.exe"  # Temporary location for the installer
...
function New-SQLExpressConfigFile {
    ...
    Write-Log -Message "Creating SQL Server 2025 Express Configuration.ini file at $ConfigFilePath" -LogFile $LogFile
...
if (Test-SQLExpressInstalled -LogFile $LogFile -InstanceName $InstanceName) {
    Write-Host "SQL Server 2025 Express ($InstanceName) is already installed. Exiting."
    Write-Log -Message "Script terminated: SQL Server 2025 Express is already installed." -LogFile $LogFile
    exit 0
}
...
Write-Log -Message "Downloading SQL Server 2025 Express from $DownloadUrl..." -LogFile $LogFile
try {
    ...
    Write-Error "Failed to download SQL Server 2025 Express. Error: $($_.Exception.Message)"
    exit 1
}
...
Write-Log -Message "Installing SQL Server 2025 Express..." -LogFile $LogFile
try {
...
# Verify installation
if (Test-SQLExpressInstalled -LogFile $LogFile -InstanceName $InstanceName) {
    Write-Host "SQL Server 2025 Express ($InstanceName) installed successfully."
    Write-Log -Message "Verification: SQL Server 2025 Express ($InstanceName) installed successfully." -LogFile $LogFile
} else {
...
Write-Host "SQL Server 2025 Express installed. Log file: $LogFile"
